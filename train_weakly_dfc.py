...
import tf_util
import pc_util
import dfc_dataset1_w_mask as dfc_dataset1
...
parser.add_argument('--existing_model', default='', help='Path to existing model to continue to train on')
parser.add_argument('--starting_epoch', type=int, default=0, help='Initial epoch number (for use when reloading model')
...
LOG_FOUT = open(os.path.join(LOG_DIR, 'train.log'), 'a')  # write in train.log
...
#         self.val_data = dfc_dataset1.DFCDataset(root=FLAGS.data_dir, npoints=self.point_sz, split='val',extra_features=FLAGS.extra_dims)
        self.test_data = dfc_dataset1.DFCDataset(root=FLAGS.data_dir, npoints=self.point_sz, split='val',extra_features=FLAGS.extra_dims)
        self.train_sz = self.train_data.__len__()
        self.val_sz = 0 #self.val_data.__len__()
    ...
    def get_batch_wdp(self, dataset, idxs, start_idx, end_idx):
        ...
        batch_ctx = np.zeros((bsize, NUM_CLASS), dtype=np.float32)
    ...
    def build_g_cpu(self):
        ...
        self.point_pl, self.label_pl, self.smpws_pl = MODEL.placeholder_inputs(2*self.batch_sz, self.point_sz, num_feat=3+len(FLAGS.extra_dims))
        ...
        self.ctx_pl = tf.placeholder(tf.float32, shape=(2*self.batch_sz, NUM_CLASS))
...
#             BATCH_PER_GPU = 2*BATCH_PER_GPU
            ...
            if 'GADH' in FLAGS.model:
                ...
                classify_loss = classify_loss * (tf.cast(tf.greater(label_slice, 0), tf.float32))
                self.Z = pred[2]
            else:
                self.Z = pred
...
#                 label_slice=tf.reshape(label_slice,[-1])
#                 smpws_slice = tf.reshape(smpws_slice,[-1])
#                 indices=tf.where(tf.not_equal(label_slice,0))
#                 label_slice=tf.cast(tf.gather(label_slice,indices),tf.int32)
#                 pred=tf.gather(pred,indices)
#                 smpws_slice=tf.gather(smpws_slice,indices)
...
#                     data_i[:,:3] = data_i[:,:3] + jitter
            ...
            ## Prepare Labels
            seg_onehot = Tool.OnehotEncode(batch_label, NUM_CLASS)
            seg_feed_onehot = []
...
#                 aug_data = provider.jitter_point_cloud(aug_data)
            ...
            seg_Onehot_feed = Tool.OnehotEncode(batch_label, NUM_CLASS)
        ...
        for l in range(NUM_CLASS):
            ...
            l, l, total_correct_class[l] / float(total_seen_class[l]), iou[l], f1[l])
        self.log_string(per_class_str)
        self.log_string('test_mIOU: {}'.format(iou[1:].mean()))
        self.log_string('test f1: {}'.format(f1[1:].mean()))
