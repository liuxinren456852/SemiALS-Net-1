...
def parse_args(argv):
    ...
    parser.add_argument('--input_path', default='./data/dfc_data/Track4/', required=False, help='Input point clouds path')
    parser.add_argument('--input_type', type=InputType, choices=list(InputType), default=InputType.TXT)
    parser.add_argument('--output_path', default='./data/dfc_data/Inference/', required=False, help='Output path')
    parser.add_argument('--output_type', type=OutputType, choices=list(OutputType), default=OutputType.LABELS)

    parser.add_argument('--radius', type=float, default=0.1, help='Radius [default: 2]')
    parser.add_argument('--sigma', type=float, default=0.05, help='BANDWIDTH [default: 1.00]')
...
#         files = glob.glob(os.path.join(FLAGS.input_path, "EVAL*.txt"))
#     elif FLAGS.input_type is InputType.LAS:
#         files = glob.glob(os.path.join(FLAGS.input_path, "EVAL*.las"))
    
    all_files = sorted(os.listdir(FLAGS.input_path))
    files = all_files[100:]
    files = [os.path.join(FLAGS.input_path, f) for f in files]
...
def pre_processor(files, input_queue):
    for file in files:
        ...
        pset = PointSet(file)
        test_data_psets = []
        [block_data_total, block_label_total] = pset.cloud2blocks_finaltest(block_size=64,stride=32)
...
#                                           block_data_total[9]), axis=0) # concat all the blocks
        ...
        for block_data, block_label in zip(block_data_total, block_label_total):
            for sub_block_data, sub_block_label in zip(block_data, block_label):
                test_data_psets.append(sub_block_data)
#                 print('sub_block_data', sub_block_data.shape)
                
        sz=test_data_psets.__len__()
        print('number of blocks', sz)
...
def main_processor(input_queue, output_queue):
    with tf.Graph().as_default():
        with tf.device('/device:GPU:' + str(GPU_INDEX)):
            pointclouds_pl, labels_pl, smpws_pl = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT, num_feat=3+len(FLAGS.extra_dims))
            is_training_pl = tf.placeholder(tf.bool, shape=())

            logging.info("Loading model")
            if 'pointconv' in FLAGS.model:
                pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, sigma=FLAGS.sigma, radius=FLAGS.radius, code=FLAGS.codeword)
            else:
...
#             LABEL_MAP: {0: 0, 1: 2, 2: 5, 3: 6, 4: 9, 5: 17}

            print('label_length: ',pset.c.__len__())
            num0 = str(pset.c.tolist()).count("0")
            num1 = str(pset.c.tolist()).count("2")
            num2 = str(pset.c.tolist()).count("5")
            num3 = str(pset.c.tolist()).count("6")
            num4 = str(pset.c.tolist()).count("9")
            num5 = str(pset.c.tolist()).count("17")
